...
class Color(str):
    ...
    def __add__(self, color2):
        ...
        return Color(str(terminal.color_from_argb(
            a1,
            min(r1 + r2, 255),
            min(g1 + g2, 255),
            min(b1 + b2, 255),
        ...
        for k, key in enumerate(keylist):
            ...
            try:
                ...
                for n in range(interp_num): #TODO xrange to range
                    ...
                    a = max(min(int(color_a[3] + ((color_b[3] - color_a[3])
                                                  * bias)), 255), 0)
                    r = max(min(int(color_a[0] + ((color_b[0] - color_a[0])
                                                  * bias)), 255), 0)
                    g = max(min(int(color_a[1] + ((color_b[1] - color_a[1])
                                                  * bias)), 255), 0)
                    b = max(min(int(color_a[2] + ((color_b[2] - color_a[2])
                                                  * bias)), 255), 0)

                    color_map.append(Color(
                        str(terminal.color_from_argb(a, r, g, b))))
                    bias += bias_inc
                current_key += 1
            except:
                pass
    ...
    def getRGB(self):
        """ Provides RGB values of name,
        usually for use in alpha transparency """
        # red = 0
        # green = 0
        # blue = 0
        # alpha = 0
    ...
    def blend(self, color2, bias=0.5):
        ...
        return Color(str(terminal.color_from_argb(a, r, g, b)))
    ...
    def trans(self, alpha_value):
        ...
        return Color(str(terminal.color_from_argb(alpha_value, r, g, b)))
